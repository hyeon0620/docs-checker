...
async def _create_user(session, *, username: str = "taro", password: str = "taro1234", role: str = "user") -> User:
...
async def test_login_wrong_password(client, session) -> None:
    await _create_user(session)
    res = await client.post("/api/login", json={"username": "taro", "password": "wrong"})
    assert res.status_code == 401


@pytest.mark.asyncio
async def test_login_unknown_user(client) -> None:
    res = await client.post("/api/login", json={"username": "ghost", "password": "x"})
    assert res.status_code == 401
...
async def test_me_with_cookie_returns_user(client, session) -> None:
    await _create_user(session)
    login = await client.post("/api/login", json={"username": "taro", "password": "taro1234"})
...
async def test_logout_clears_cookie(client, session) -> None:
    await _create_user(session)
    await client.post("/api/login", json={"username": "taro", "password": "taro1234"})
